...
from src.binaryconnect import BC
from models.resnet import ResNet18

class WandB:
    project_name = "deep-learning-binary-train"
    run_name = "training"
    job_type = "training_test"

#############################################################################################################################################################
def binary_train(args):
    # Initialize Weights & Biases
    wandb.init(project=WandB().project_name, config=args.__dict__ , name=WandB().run_name, job_type=WandB().job_type)
    ...
    for epoch in range(args.epochs):
        ...
        for images, labels in trainloader:
            ...
            # Binarize weights before forward pass
            ...
            # Restore full precision after backpropagation
            loss.backward()
            optimizer.step()
            mymodelbc.restore()
